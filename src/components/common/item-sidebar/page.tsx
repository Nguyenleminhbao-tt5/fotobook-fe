...
const ItemSidebar= ({isGroup,source, title, size, isStory }: Props)=>{
    return (
        <div className="w-full hover:bg-[#3A3B3C] h-[68px] flex items-center rounded-[10px] ps-2 my-2">
            {
                isGroup? <img className={`${size=="small"? 'h-[40px] w-[40px]': 'h-[48px] w-[48px]'}  rounded-[10px] mr-4`} src={source}/> :
                <span className="relative">
                    <img src={source} className={`h-[45px] w-[45px] rounded-full mr-4    ${isStory && "border-[#0866FF] p-[2px] border-[2px]"} `}/>
